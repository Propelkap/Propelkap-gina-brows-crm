...
import { Check, XCircle, Clock, Trash2, CreditCard } from "lucide-react";
import { useRouter } from "next/navigation";
import StripeLinkButton from "./StripeLinkButton";

export default function CitaActions({
  citaId, estadoActual, precioMxn, clienteWhatsapp,
}: {
  citaId: string;
  estadoActual: string;
  precioMxn?: number;
  ...
  return (
    <div className="mt-2">
      <div className="flex flex-wrap gap-2">
        ...
        {precioMxn != null && (
          <button
            ...
            className="text-xs inline-flex items-center gap-1 px-3 py-1 rounded-full bg-[var(--secondary)]/40 text-[var(--primary-dark)] hover:bg-[var(--secondary)]/70 transition-colors"
          >
            <CreditCard className="w-3 h-3" /> Cobrar anticipo
          </button>
        )}
        <button
      ...
      {showStripe && precioMxn != null && (
        <StripeLinkButton citaId={citaId} precioMxn={precioMxn} clienteWhatsapp={clienteWhatsapp ?? null} />
      )}
    </div>
  );
}
